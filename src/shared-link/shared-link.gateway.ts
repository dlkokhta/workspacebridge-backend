...
import { JoinSharedLinkRoomDto } from './dto/join-shared-link-room.dto';
...
export class SharedLinkGateway
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  @SubscribeMessage('joinSharedLinksRoom')
  async handleJoinRoom(
    @ConnectedSocket() client: AuthenticatedSocket,
    @MessageBody() body: JoinSharedLinkRoomDto,
  ) {
  ...
  // Called by SharedLinkService after a successful mutation. Broadcasts to
  // every connected member of the workspace so the other side updates
  // without a refresh.
  emitLinkCreated(workspaceId: string, link: unknown) {
    this.server.to(this.roomFor(workspaceId)).emit('sharedLinkCreated', link);
  }

  emitLinkDeleted(workspaceId: string, linkId: string) {
    this.server
      .to(this.roomFor(workspaceId))
      .emit('sharedLinkDeleted', { id: linkId });
  }
